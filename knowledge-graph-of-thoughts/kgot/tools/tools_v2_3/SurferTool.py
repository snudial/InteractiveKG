...
try:
    ...
    WIKIPEDIA_TOOL_AVAILABLE = True
except (ImportError, ModuleNotFoundError) as e:
    WIKIPEDIA_TOOL_AVAILABLE = False
    logger.warning(f"WikipediaTool not available (missing dependency): {e}")
    ...
    def __init__(self, model_name: str, temperature: float, usage_statistics: UsageStatistics, **kwargs: Any):
        ...
        if WIKIPEDIA_TOOL_AVAILABLE:
            try:
                ...
                logger.warning(f"Failed to initialize Wikipedia tool: {e}. Continuing without it.")
                wikipedia_tool = None
        else:
            wikipedia_tool = None
